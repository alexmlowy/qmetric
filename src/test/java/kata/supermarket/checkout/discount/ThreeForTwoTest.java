...
import kata.supermarket.checkout.discount.single_product.ThreeForTwo;
...
class ThreeForTwoTest {
    ...
    void basketProvidesTotalDiscount(String description, String expectedDiscount, EnumSet<ItemName> applicableItems, Basket basket) {
        final Discount discount = new ThreeForTwo(applicableItems, PRICE_LIST);
    ...
    static Stream<Arguments> basketProvidesTotalDiscount() {
        return Stream.of(
                ...
                notApplicableOnTwoItems(),
                discountAppliedOnThreeMatchingItems(),
                discountAppliedOnlyOncePerMatchingGroup(),
                discountAppliedMultipleTimesOnMatchingGroups(),
                discountAppliedOnMultipleItems()
        );
    }

    private static Arguments noApplicableItems() {
        return Arguments.of("no applicable items", "0", EnumSet.noneOf(ItemName.class),
                createBasket(Milk, Milk));
    }

    private static Arguments noMatchingItems() {
        return Arguments.of("no matching items", "0", EnumSet.of(Pack_Of_Digestives),
                createBasket(Milk, Milk));
    }

    private static Arguments notApplicableOnSingleItem() {
        return Arguments.of("not applicable on single item", "0", EnumSet.of(Milk),
                createBasket(Milk));
    }

    private static Arguments notApplicableOnTwoItems() {
        return Arguments.of("not applicable on two items", "0", EnumSet.of(Milk),
                createBasket(Milk, Milk));
    }

    private static Arguments discountAppliedOnThreeMatchingItems() {
        return Arguments.of("discount applied on three matching items", "0.49", EnumSet.of(Milk),
                createBasket(Milk, Milk, Milk));
    }

    private static Arguments discountAppliedOnlyOncePerMatchingGroup() {
        return Arguments.of("discount applied only once per matching group", "0.49", EnumSet.of(Milk),
                createBasket(Milk, Milk, Milk, Milk));
    }

    private static Arguments discountAppliedMultipleTimesOnMatchingGroups() {
        return Arguments.of("discount applied multiple times on matching groups", "0.98", EnumSet.of(Milk),
                createBasket(Milk, Milk, Milk, Milk, Milk, Milk));
    }

    private static Arguments discountAppliedOnMultipleItems() {
        return Arguments.of("discount applied on multiple items", "2.04", EnumSet.of(Milk, Pack_Of_Digestives),
                createBasket(Milk, Milk, Milk, Pack_Of_Digestives, Pack_Of_Digestives, Pack_Of_Digestives));
    }
